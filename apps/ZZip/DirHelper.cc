...
DirHelper::DirHelper(const tstring& sPath, bool bSubFolder) 
: sFolderPath_(sPath)
, bSubFolder_(bSubFolder)
{
} 

bool DirHelper::SetInitDir(const tstring& sDir) 
{ 
	//先把dir转换为绝对路径 
...
bool DirHelper::BeginBrowse(const tstring& sFilespec) 
{ 
	ProcessDir(sFolderPath_,NULL); 
	return BrowseDir(sFolderPath_, sFilespec); 
} 

bool DirHelper::BrowseDir (const tstring& sDir, const tstring& sFilespec) 
{ 
	_tchdir(sDir.c_str()); 
...
bool DirHelper::ProcessFile(const tstring& sFileName) 
{
	return true; 
} 

void DirHelper::ProcessDir(const tstring& sCurrentDir, const tstring& sParentDir) {}